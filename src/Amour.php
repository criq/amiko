...
class Amour {
	...
	static function success($res = NULL) {
		...
		return \Katu\Utils\JSON::respond($res);
	}
	...
	static function error($error = NULL) {
		...
		return \Katu\Utils\JSON::respond(array(
			'error' => array(
				'message' => $error,
			),
		));
	...
	static function getURL($endpoint, $params) {
		return \Katu\Utils\URL::joinPaths(Config::getApp('apiURL'), $endpoint) . '?' . http_build_query($params);
	}
